...
According to Ministry of Sustainability and the Environment, the daily mean temperature is projected to increase by 1.4 to 4.6 degrees celsius by the end of the century.
...
pacman::p_load(tidyverse, ggdist, plotly, gganimate, DT, crosstalk, ggstatsplot, nortest, ggiraph, hrbrthemes, ggpubr)
...
Singapore’s climate station record for temperature stretches back to 1929. The location of the climate station has changed over time; it has been at its current location at Changi Airport since 1984. Therefore, the most reliable long-term records for assessing impact of climate change, urbanisation and other factors on the climate station are for the 1984 – 2023 period, due to earlier changes in the climate station location affecting the measurements.[Source](https://www.weather.gov.sg/wp-content/uploads/2023/03/ACAR_2022.pdf)
...
c1 <- ggplot(data = d1, 
  ...
  xlab("Day") + 
  ylab("Daily Temperature (Degrees celsius)") +
...
crosstalk::bscols(widths = c(7,5), gg1,               
                  datatable(d1, caption = "Daily Temperature over the Years", class='compact'))      
```
...
However, the line graph was not very useful to help us see if there is a rising trend in daily temperature since we are looking at daily temperature in the month of August in Singapore and we are only looking at the mean daily temperature in the Weather.gov.sg data. As such, we added an interactive boxplot. A boxplot not only tells us the dispersion and variation in the data, it also tells us the minimum and maximum value. In addition, we can also add jitter to boxplot to get an idea of the "concentration" of the data points. If the temperature is increasing as years pass, we should be able to observe that there are higher "concentration" of data points at the higher temperatures and the temperature range for each year would be going up. 
...
fig <- subplot(gg2, gg1, widths = c(0.3, 0.7)) %>% 
  layout(title = 'Daily Temperature in the Month of August',
...
From the line graph, the distributions of the daily temperature for all five years seem to be similar due to the daily temperature being broken down into days. 
...
In addition, from the boxplot, we also see that the temperature ranges for each year was different. For example, it seems that the daily temperature for 2003, 2013 and 2023 have similar temperature range, where the lowest daily temperature is around 26.5 to 26.7 degrees Celsius, and the highest daily temperature is around 29 to 29.5 degree Celsius. Interestingly, 1993 experienced a wide range of daily mean temperature. 1993 had the lowest temperature of 25.5 degrees Celsius and also experienced the highest temperature of 30.1 degree Celsius. These two temperatures were also the lowest and highest temperatures out of these five years.
...
With median as point estimate, quantile intervals are used instead of confidence interval. We use 95% and 99% intervals because they are commonly associated with 5% and 1% error rate, which are commonly used in hypothesis testing. 
...
To plot this raincloud plot, we:
...
c4 <- plot_ly(data = d2,
        ...
        type = "scatter",
        mode = "lines+markers") |>
  layout(title = "Temperature observed by \nChangi Weather Station in August",
...
gg4 <- highlight(ggplotly(c4),        
                "plotly_selected")  

gg5 <- highlight(ggplotly(c5),
                 "plotly_selected")

crosstalk::bscols(widths = c(6,6), gg4, gg5)  
...
d3 <- highlight_key(changitemp_transformed) 
...
c1 <- ggplot(data = d3, 
  ...
  theme_ipsum_rc(plot_title_size = 13, plot_title_margin=4,
                         subtitle_size=11, subtitle_margin=4, 
                         axis_title_size = 8, axis_text_size=8,
                 axis_title_face="bold", plot_margin = margin(30, 30, 30, 30)) + 
   theme(panel.grid.major = element_line(size = 0.8, color = "grey90")) +
  theme(panel.grid.minor = element_line(size = 0.8, color = "grey90"))
