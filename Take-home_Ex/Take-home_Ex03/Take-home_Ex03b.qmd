...
datatable(changitemp_transformed, rownames = FALSE)
```

# Exploratory Data Analysis

## Exploring the Distribution of Daily Temperature

To get a sense of the trend of the daily temperature in these five years, we first plot a line graph. 
...
c1 <- ggplot(data = changitemp, 
  ...
  #annotate("text", x = 20, y = mean_at$mean_annual_temp, label = mean_at$mean_annual_temp, vjust = -0.5) + 
...
```
From the above, we 
However, the line graph was not very useful since we are looking at daily temperature in the month of August in Singapore and we are only looking at the mean daily temperature in the Weather.gov.sg data. As such, we added a boxplot and made both the boxplot and line chart interactive. 
...
datatable(resultlist, rownames = FALSE, caption = "p-value for each Year's Anderson-Darling test", class='compact')
...
We will use ggstatsplot's `ggbetweenstats()` to visualise the ANOVA Test results. As we were unable to confirm the normality assumption for the distribution of daily temperature, non-parametric test is considered, hence `type = np`. 

Comparisons were made pairwise between significant pairs since we are interested to know those pairs with significant difference between them. Hence, `pairwise.display = "s" `. 
...
Since the p-value (0.0008444241) is less than the critical value of 0.05, there is statistical evidence to reject the null hypothesis. We can conclude that for each year (grouped by daily temperatures in each year), there is difference between median daily temperature.

In the above plot, we also observed that there are certain pairs of years with p-value less than 0.05. These pairs are: 1983 and 2023, 1993 and 2013, 2013 and 2023. This suggests that the differences between the medians of these pairs are statistically significant.
