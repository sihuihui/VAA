...
```{r}
monthly_avgmaxtemp <- period.apply(changi_ts$maximum_temperature, INDEX = seq(1, nrow(changi_ts) - 1, 30.4375), FUN = colMeans)

plot(ts_ts(monthly_avgmaxtemp), col="darkred", ylim=c(20, 40), 
...
```{r}
# create samples 
training <- ts_ts(window(monthly_avgmaxtemp, start = as.Date('1983-01-01'), end = as.Date('2015-12-31')))
validation <- ts_ts(window(monthly_avgmaxtemp, start = as.Date('2016-01-01')))
...
```
```{r}
autoplot(monthly_avgmaxtemp, ts.colour = "blue")
...
```{r}
plot(monthly_avgmaxtemp, col="blue", xlab="Year", ylab="Daily Max Temp", main="ETS Forecast", type='l') 
