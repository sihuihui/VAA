...
As the charts that we will be re-creating only uses the selected columns, I also drop the columns that we will not be using for this exercise using the following code chunk.
...
-   The original author seems to want to see if there are any differences in subject scores between gender.
...
Let us first prepare the data for each subject. When preparing the data, I removed the "70200" in the school IDs (i.e. `CNTSCHID`) because it is present in every school IDs and they would take up extra space and cause clutter when we plot the school IDs in the axis.
...
I decided to bind the scores into proficiency levels and plot ridgeplots to compare the students' proficiency levels across ESCS. This is how I imagined the revamped chart would look like:

![Finalised Suggestion for Chart 4](images/finalised4.png){fig-align="center"}

### Makeover Design

First, we will prepare the data by binning the subject scores into various proficiency levels and also defining the factor levels for the subject's proficiency levels.
...
With the prepared data, we will plot 1 chart for each subject. When working on the chart, I realised that a stat_halfeye from ggdist is better than ridge chart from ggridge because it shows the density and the interval with a boxplot.
...
We will now use patchwork to stitch these 3 charts into 1 plot, add title, subtitle, caption and also add a theme to the plot.
