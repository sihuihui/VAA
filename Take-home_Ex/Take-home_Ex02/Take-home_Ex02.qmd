...
Before I can critique and remake my peer's charts, I followed the following data preparation steps taken by my peer.
...
![Shortlisted Chart for Original Chart 1](images/Picture2.png){fig-align="center" width="543"}
...
ggplot(stu1, 
  ...
  stat_summary(fun = max, geom = "text", aes(label = paste(round(after_stat(y), 0))),
               position = position_nudge(y=-0.25), vjust=-0.5, size = 2, color = "grey40")+
  labs(title = "Scores varied widely for each subject.\nThe difference in median scores between were small.", subtitle = "Number of observations: 6606", caption = "Data from PISA 2022") + 
  xlab("SUBJECT") + 
  ylab("SCORES") + 
...
-   Notched boxplots were also used to display the difference in subject scores between gender.
...
![Finalised Chart for Chart 2](images/finalised2.png){fig-align="center" width="460"}

### Makeover Design

Let us prepare the data for this chart. We will first select the columns that we want, then `pivot_longer()` so that we can plot all the subjects scores for both gender into 1 plot.
...
-   The original author seems to want to see if there are any differences in subject scores between schools.
...
-   In the original work, the students' scores were averaged to determine the school's score but averages are affected by extreme values.
