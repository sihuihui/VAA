---
title: "Exploring and Forecasting Time Series using timeTK and Modeltime"
author: "Goh Si Hui"
date: 2024/03/17
...
# What we want to do 
We want to find the most suitable package(s) that can: 
1.    allow us to easily aggregate daily data to monthly data
2.    impute missing values 
3.    do decomposition of time series data 
4.    plot time series data
5.    do forecasting of time series data 
...
datacleaned <- data %>%
  select(station, tdate, mean_temperature, maximum_temperature, minimum_temperature, daily_rainfall_total) %>%
  drop_na(tdate)
...
weatherdata <- weatherdata %>%
  filter_by_time(tdate, .start_date = "2014-01", .end_date = "2023-12")
...
weatherdata %>%
  group_by(station) %>%
  plot_time_series(tdate, mean_monthly_temperature, .facet_ncol = 3)
```
...
weatherdata %>%
  group_by(station) %>%
  plot_time_series(tdate, min_monthly_temperature, .facet_ncol = 3)
```
...
weatherdata %>%
  group_by(station) %>%
  plot_time_series(tdate, max_monthly_temperature, .facet_ncol = 3)
```
...
weatherdata %>%
  group_by(station) %>%
  plot_time_series(tdate, monthly_rainfall, .facet_ncol = 3)
```
...
weatherdata %>%
  ...
  plot_seasonal_diagnostics(tdate, mean_monthly_temperature)
...
weatherdata %>%
  filter(station == "Admiralty") %>%
  plot_seasonal_diagnostics(tdate, monthly_rainfall)
...
weatherdata %>%
  filter(station == "Admiralty") %>%
  plot_stl_diagnostics(tdate, mean_monthly_temperature,
...
weatherdata %>%
  filter(station == "Admiralty") %>%
  plot_stl_diagnostics(tdate, min_monthly_temperature,
                       .frequency = "auto", .trend = "auto",
                        .feature_set = c("observed", "season", "trend", "remainder"))
...
weatherdata %>%
  filter(station == "Admiralty") %>%
  plot_stl_diagnostics(tdate, max_monthly_temperature,
                       .frequency = "auto", .trend = "auto",
                        .feature_set = c("observed", "season", "trend", "remainder"))
...
weatherdata %>%
  filter(station == "Admiralty") %>%
  plot_stl_diagnostics(tdate, monthly_rainfall,
                       .frequency = "auto", .trend = "auto",
                        .feature_set = c("observed", "season", "trend", "remainder"))
...
Going to develop a few basic models 
-   ARIMA
...
weatherdata_meantemp <- weatherdata %>%
  filter(station == "Admiralty") %>%
  select(date = tdate, value = mean_monthly_temperature)
...
weatherdata_rf <- weatherdata %>%
  filter(station == "Admiralty") %>%
  select(date = tdate, value = monthly_rainfall)
